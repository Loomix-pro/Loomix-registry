...
export default function NotFoundPage() {
  return (
    <div className="w-full min-h-[calc(100vh-64px)] bg-black overflow-hidden flex justify-center items-center relative">
      <MessageDisplay />
      <CharactersAnimation />
      <CircleAnimation />
    </div>
  ...
  return (
    <div className="absolute flex flex-col justify-center items-center w-[90%] h-[90%] z-[100]">
      ...
      >
        ...
        <div className="text-[15px] w-full max-w-[600px] text-center text-white/80 m-[1%]">
          {t("description")}
        </div>
        <div className="flex gap-6 mt-8">
          <button
            onClick={() => router.back()}
            className="text-white border-2 border-white hover:bg-white hover:text-black transition-all duration-300 ease-in-out px-6 py-2 h-auto text-base font-medium flex items-center gap-2 hover:scale-105 rounded-xl group"
          >
            <svg
          ...
          <button
            onClick={() => router.push("/")}
            className="bg-white text-black hover:bg-gray-200 transition-all duration-300 ease-in-out px-6 py-2 h-auto text-base font-medium flex items-center gap-2 hover:scale-105 rounded-xl group"
          >
            <svg
              xmlns="http://www.w3.org/2000/svg"
              width="20"
...
function CharactersAnimation() {
  ...
  useEffect(() => {
    ...
    stickFigures.forEach((figure, index) => {
      ...
      stick.style.filter = "invert(1)" // Invert colors because background is black
  ...
  useEffect(() => {
    const handleResize = () => {
      if (charactersRef.current) {
        charactersRef.current.innerHTML = ""
        // Re-create animations after resize - this is a bit tricky to dispatch properly in React,
        // but since we only care about CSS layout, the browser handles position:absolute % fine.
      }
    }
