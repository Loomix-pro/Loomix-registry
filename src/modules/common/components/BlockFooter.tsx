...
const BlockFooter = React.forwardRef<HTMLDivElement, BlockFooterProps>(
  ({ title, badge, description, buttonText, buttonLink, className, ...props }, ref) => {
    const hasContent = title || badge || description || (buttonText && buttonLink)
    if (!hasContent) return null

    return (
      <div
        ref={ref}
        className={`w-full max-w-5xl mx-auto px-6 pt-3 pb-5 flex flex-col sm:flex-row items-start sm:items-center justify-between gap-6 border-t border-border/40 ${className || ""}`}
        dir="rtl"
        {...props}
      >
        ...
        {/* Left side: CTA Button (replacing the percentage from reference image) */}
